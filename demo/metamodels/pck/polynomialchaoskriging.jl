...
pce_trend = SurrogateModelling.PolynomialChaosExpansion(data_aug_train, y_symbol, bases, degree; solver=LASSOSolver())
pck = SurrogateModelling.PolynomialChaosKriging(data_aug_train, y_symbol, pce_trend)


fit!(pck)

# 5. prediction capacity
y_pred, y_pred_σ = predict(pck, W_aug_test) 

rmse_val = rmse(y_test, y_pred)
q2_val = q2(y_test, y_pred)

println("PCK RMSE: $(round(rmse_val, digits=3))")
println("PCK Q2: $(round(q2_val, digits=3))")


n_pool = 100
data_aug_pool, data_phys_pool = build_augmented_design(physical_model, specs, n_pool)

SurrogateModelling.evaluate!(pck, data_aug_pool; mode=:sample, n_samples=5)