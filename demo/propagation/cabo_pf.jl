...
x1 = RandomVariable(
    ProbabilityBox{Normal}(Dict(:μ => Interval(0.1, 0.2), :σ => 0.1)), :x1
)
specs = InputSpec.([x1])

physical_model = Model(df -> sin.(3 .* df.x1) .+ 0.2 .* df.x1 .^ 2, :y)
function epistemic_model(μ; σ=0.1) 
    return cdf(Normal(), (0 - μ)/σ) - cdf(Normal(), (-0.963031605 - μ)/σ)
end
...
cabo_min = cabo_loop(
    ...
    Ng=200, Nx=500, qoi_type=:pf, y_star = 0.0,
...
true_min = ([0.20], 0.02275)
true_max = ([ 0.10],  0.1586)
